...
<head>
    ...
    <title>Puestos Comerciales | La Vitrina</title>
...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 sm:px-0">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h3 class="text-2xl font-bold text-slate-800">Puestos Comerciales</h3>
                <p class="text-sm text-slate-500 mt-1">Gestiona los puestos comerciales del sistema</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
            <form method="GET" class="p-5 border-b border-slate-100 bg-slate-50/50">
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" placeholder="Buscar puesto, sucursal o número..." 
            ...
            <?php if (empty($puestosComerciales)): ?>
                <div class="p-12 text-center">
                    <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 mb-4">
                        <i class="fa-solid fa-shop text-2xl text-slate-400"></i>
                    </div>
                    <h4 class="text-lg font-semibold text-slate-700 mb-2">No hay puestos comerciales registrados</h4>
                    <p class="text-slate-500">Comienza agregando un nuevo puesto</p>
                </div>
            <?php else: ?>
                    ...
                    <table class="min-w-full">
                        <thead class="bg-slate-800 text-white">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Tipo Puesto</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Sucursal</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Interior/Número</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Observación</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php foreach ($puestosComerciales as $index => $puesto): ?>
                                <tr class="<?php echo $index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'; ?> hover:bg-blue-50/50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-700">
                                        #<?php echo htmlspecialchars($puesto['id_puesto_comercial']); ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                                        <?php echo htmlspecialchars($puesto['tipoPuesto']); ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                                        <?php echo htmlspecialchars($puesto['sucursal']); ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">
                                        <?php echo htmlspecialchars($puesto['interior']); ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                        <?php echo htmlspecialchars($puesto['observacion']); ?>
                                    </td>
            ...
            <?php if (!empty($puestosComerciales) && $total_pages > 1): ?>
                <?php 
...
<!-- Modal Nuevo/Editar Puesto Comercial -->
<div id="puestoModal" class="fixed inset-0 z-50 hidden">
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm" onclick="closeModal()"></div>
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-slate-800" id="modalTitle">Nuevo Puesto Comercial</h3>
                ...
                <form id="puestoForm" method="POST" action="/mantenimiento/puesto/guardar">
                    <input type="hidden" name="id" id="puestoId">
                    <div class="space-y-4">
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="interior" id="interior" required class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Número de puesto">
                        </div>
                        <div>
                            <label for="observacion" class="block text-sm font-medium text-slate-700 mb-1">Observación</label>
                            <textarea name="observacion" id="observacion" rows="3" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Observaciones"></textarea>
                        </div>
                        <div>
...
<script>
    function openNewModal() {
        document.getElementById('modalTitle').textContent = 'Nuevo Puesto Comercial';
        document.getElementById('puestoId').value = '';
        document.getElementById('tipoPuesto').value = '';
    ...
    function confirmDelete(id, nombre) {
        document.getElementById('deleteMessage').textContent = '¿Está seguro de eliminar el puesto "' + nombre + '"? Esta acción no se puede deshacer.';
        document.getElementById('deleteBtn').href = '/mantenimiento/puesto/eliminar?id=' + id;
